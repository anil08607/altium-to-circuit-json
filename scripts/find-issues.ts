    ...
    for (const record of doc.records) {
      ...
      if (record instanceof AltiumTrackRecord) {
        if (isKeepoutLayer(layer)) issues.add("UnsupportedKeepoutTrack")
          ...
          !isCourtyardLayer(layer) &&
          !isOverlayLayer(layer) &&
          !mapAltiumCopperLayer(layer)
        ) {
      ...
      if (record instanceof AltiumPadRecord) {
        const shape = (record.shape ?? "ROUND")
          .replace(/[\s_-]+/gu, "")
          .toUpperCase()
        if (
          !["ROUND", "CIRCLE", "OVAL", "SQUARE"].includes(shape) &&
          !shape.includes("RECT") &&
          !shape.includes("OCTAGON")
        ) {
